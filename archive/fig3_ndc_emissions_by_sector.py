...
def generate_fig3_ndc_emissions_by_sector(df: pd.DataFrame, output_dir: str) -> None:
    print("generating figure 3 — emissions on y-axis, only blue/red points")
    ...
    pivot["label"] = pivot.index + " (" + pivot["diff"].round(0).astype(int).astype(str) + ")"
    ...
    x_pos = {label: i for i, label in enumerate(ordered_label_list)}

    fig = go.Figure()

    part_2024 = data[data["Year"] == 2024]
    fig.add_trace(go.Scatter(
        x=part_2024["SectorLabel"],
        y=part_2024["CO2eq"],
        mode="markers",
        name="2024",
    ...
    fig.update_yaxes(showgrid=True, title="CO₂eq (kt)", rangemode="tozero")
    fig.update_layout(
