# charts/chart_generators/fig5_2_1_sarem_twh.py
...
def generate_fig5_2_1_sarem_twh(df: pd.DataFrame, output_dir: str) -> None:
    """
    Generate stacked bar chart for SAREM comparison of TWh generation
    in 2024, 2030, and 2035.
    """
    print("generating figure 5.2.1-9: SAREM generation comparison (TWh)")
    ...
    if dev_mode:
        ...
        save_figures(fig, output_dir, name="fig5_2_1_sarem_twh")

        # Copy main image into gallery
        gallery_dir = project_root / "outputs" / "gallery"
        gallery_dir.mkdir(parents=True, exist_ok=True)
        src_img = Path(output_dir) / "fig5_2_1_sarem_twh_report.png"
        if src_img.exists():
            shutil.copy(src_img, gallery_dir / src_img.name)

        # Export CSV
        Path(output_dir).mkdir(parents=True, exist_ok=True)
        df.to_csv(Path(output_dir) / "fig5_2_1_sarem_twh_data.csv", index=False)


if __name__ == "__main__":
    data_path = project_root / "data" / "processed" / "5.2.1_8_pwr_sarem_TWh.csv"
    df = pd.read_csv(data_path)

    out = project_root / "outputs" / "charts_and_data" / "fig5_2_1_sarem_twh"
    out.mkdir(parents=True, exist_ok=True)
    generate_fig5_2_1_sarem_twh(df, str(out))
