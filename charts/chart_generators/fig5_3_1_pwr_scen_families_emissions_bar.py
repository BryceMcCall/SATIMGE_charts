# charts/chart_generators/fig5_3_3_pwr_scen_families_bar_emissions_mtco2eq.py
...
# ───────────────────────── Generator ─────────────────────────
def generate_fig5_3_3_pwr_scen_families_bar_emissions_mtco2eq(df: pd.DataFrame, output_dir: str) -> None:
    """
    Generate stacked bar chart showing emissions (MtCO₂-eq)
    across scenario families for milestone years (2024, 2030, 2035).
    """
    print("generating figure 5.3.3: Power Sector Scenario Families Emissions (MtCO₂-eq)")
    ...
    # Keep only desired families (same order as capacity/TWh)
    ...
    fig.update_layout(
        ...
        font=dict(size=14),
        margin=dict(l=100, r=180, t=40, b=120),
        yaxis=dict(
            ...
            minor=dict(
                ...
                dtick=5           # minor gridlines every 5 MtCO₂-eq
            ),
        ),
        bargap=0.45
    )
    ...
    if dev_mode:
        ...
        save_figures(fig, output_dir, name="fig5_3_3_pwr_scen_families_bar_emissions_mtco2eq")

        gallery_dir = project_root / "outputs" / "gallery"
        gallery_dir.mkdir(parents=True, exist_ok=True)
        src_img = Path(output_dir) / "fig5_3_3_pwr_scen_families_bar_emissions_mtco2eq_report.png"
        if src_img.exists():
            shutil.copy(src_img, gallery_dir / src_img.name)

        df.to_csv(Path(output_dir) / "fig5_3_3_pwr_scen_families_bar_emissions_mtco2eq_data.csv", index=False)


# ───────────────────────── Entry Point ─────────────────────────
if __name__ == "__main__":
    data_path = project_root / "data" / "processed" / "5.3.1_pwr_scen_families_emissions_bar_fuels.csv"
    df = pd.read_csv(data_path)

    out = project_root / "outputs" / "charts_and_data" / "fig5_3_3_pwr_scen_families_bar_emissions_mtco2eq"
    out.mkdir(parents=True, exist_ok=True)
    generate_fig5_3_3_pwr_scen_families_bar_emissions_mtco2eq(df, str(out))
