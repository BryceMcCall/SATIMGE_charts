# charts/chart_generators/fig5_2_3_ee_emissions_stacked_bar_ipcc1.py
...
# ───────────────────────── Generator ─────────────────────────
def generate_fig5_2_3_ee_emissions_stacked_bar_ipcc1(df: pd.DataFrame, output_dir: str) -> None:
    """
    Generate stacked bar chart comparing WEM vs WEM + EE emissions (MtCO₂-eq)
    for all years (2025–2035), faceted by year horizontally.
    Legend placed below.
    """
    print("generating figure 5.2.3: Energy Efficiency Emissions comparison (WEM vs WEM + EE)")
    ...
    if not dev_mode:
        save_figures(fig, output_dir, name="fig5_2_3_ee_emissions_stacked_bar_ipcc1")
        df.to_csv(Path(output_dir) / "fig5_2_3_ee_emissions_stacked_bar_ipcc1_data.csv", index=False)


if __name__ == "__main__":
    data_path = project_root / "data" / "processed" / "5.2.1_EE_emissions_stacked_bar_ipcc1.csv"
    out = project_root / "outputs" / "charts_and_data" / "fig5_2_3_ee_emissions_stacked_bar_ipcc1"
    out.mkdir(parents=True, exist_ok=True)
    generate_fig5_2_3_ee_emissions_stacked_bar_ipcc1(pd.read_csv(data_path), str(out))
