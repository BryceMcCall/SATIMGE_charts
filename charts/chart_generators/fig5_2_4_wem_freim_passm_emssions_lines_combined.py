# fig5_2_1_wem_freim_passm_lines.py
...
# ───────────────── Generator ────────────────────
def generate_fig5_2_1_wem_freim_passm_lines(df: pd.DataFrame, output_dir: str) -> None:
    """
    Combined emissions line: WEM, WEM + Freight modal shift, WEM + Passenger modal shift.
    ...
    # Map labels and keep only the three of interest
    df["ScenarioLabel"] = df["Scenario"].apply(_nice_label)
    keep = ["WEM", "WEM + Freight modal shift", "WEM + Passenger modal shift"]
    df = df[df["ScenarioLabel"].isin(keep)].copy()

    # Legend order: alternatives first, WEM last
    ...
    # Major + minor ticks AND minor grid lines
    fig.update_xaxes(
        range=[2024, 2035 + 0.03],
        tickmode="linear", tick0=2024, dtick=1,
        ticks="outside",
        showgrid=True,                    # major grid
        minor=dict(ticks="outside", dtick=0.5, showgrid=True),  # minor ticks + grid
    ...
    fig.update_yaxes(
        ticks="outside",
        showgrid=True,                    # major grid
        minor=dict(ticks="outside", dtick=5, showgrid=True),    # minor ticks + grid
        title="Emissions (MtCO₂-eq)",
        title_font=dict(size=AXIS_TITLE_SIZE),
        tickfont=dict(size=AXIS_TICK_SIZE),
    )

    # Visual guide at 2035
    ...
    out = Path(output_dir)
    if not DEV_MODE:
        save_figures(fig, str(out), name="fig5_2_1_wem_freim_passm_lines")
        out.mkdir(parents=True, exist_ok=True)
        df.to_csv(out / "fig5_2_1_wem_freim_passm_lines_data.csv", index=False)
        gal = PROJECT_ROOT / "outputs" / "gallery"
        gal.mkdir(parents=True, exist_ok=True)
        png = out / "fig5_2_1_wem_freim_passm_lines_report.png"
        if png.exists():
            shutil.copy2(png, gal / png.name)

# ──────────────── CLI (convenience) ────────────────
if __name__ == "__main__":
    primary = PROJECT_ROOT / "data" / "processed" / "5.2.1_wem_freim_passm_lines.csv"
    if primary.exists():
        df0 = pd.read_csv(primary)
    else:
        fallback = PROJECT_ROOT / "data" / "processed" / "5.2.1_FreiM_PassM_emissions_line.csv"
        if not fallback.exists():
            raise SystemExit("No input CSV found for combined WEM/FreiM/PassM plot.")
        df0 = pd.read_csv(fallback)

    out_dir = PROJECT_ROOT / "outputs" / "charts_and_data" / "fig5_2_1_wem_freim_passm_lines"
    out_dir.mkdir(parents=True, exist_ok=True)
    generate_fig5_2_1_wem_freim_passm_lines(df0, str(out_dir))
